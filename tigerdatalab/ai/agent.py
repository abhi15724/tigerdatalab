...
@dataclass
class CompanyAgent:
    """High-level facade for building and deploying a company-specific AI agent."""
    ...
    def prepare_training(self, source: Any, *, task: str = "sft", output_dir: str | Path | None = None,
                         min_chars: int = 1, max_chars: int | None = None) -> AIDataset:
    ...
    def train(self, model: str, *, output_dir: str | Path, dataset: Any | None = None,
              backend: Any = "auto", task: str = "sft", **kwargs: Any) -> Any:
    ...
    def connect(self, provider: Provider, model: str, *, system: str | None = None) -> "CompanyAgent":
        ...
        self.ai = CompanyAI(router, knowledge_base=self.knowledge_base, tools=self.tools,
                            workflow=self.workflow, evaluator=self.evaluator)
        self._system = system
        return self
    ...
    def deploy(self, *, host: str = "0.0.0.0", port: int = 8000, version: str = "1.0.0", **kwargs: Any) -> None:
        """Serve this ready agent as a real-time HTTP API using FastAPI/Uvicorn.

        Install the optional deployment dependencies with
        ``pip install 'tigerdatalab[deployment]'``. For production, place the
        service behind TLS/authentication/rate limiting and a process manager
    ...
    def app(self, *, version: str = "1.0.0") -> Any:
        """Return the ASGI application for Docker, Gunicorn/Uvicorn or cloud deployment."""
